...
def get_possible_moves(point, maze_walls, size_x, size_y, add=True):
    ...
    if direction == ">":
        possible_moves = [(0, 1, 1 if add else -1, ">", length), (-1, 0, 1001 if add else -1001, "^", length), (1, 0, 1001 if add else -1001, "v", length)]
    elif direction == "<":
        possible_moves = [(0, -1, 1 if add else -1, "<", length), (-1, 0, 1001 if add else -1001, "^", length), (1, 0, 1001 if add else -1001, "v", length)]
    elif direction == "^":
        possible_moves = [(-1, 0, 1 if add else -1, "^", length), (0, -1, 1001 if add else -1001, "<", length), (0, 1, 1001 if add else -1001, ">", length)]
    elif direction == "v":
        possible_moves = [(1, 0, 1 if add else -1, "v", length), (0, -1, 1001 if add else -1001, "<", length), (0, 1, 1001 if add else -1001, ">", length)]
...
def execute_part_one(input: list[str]) -> None:
    ...
    print(f"Solved 1: {scores[maze_end[0]][maze_end[1]]}")
